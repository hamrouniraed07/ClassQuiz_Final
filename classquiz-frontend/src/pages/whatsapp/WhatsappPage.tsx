...
function CustomSelect({ value, onChange, options, placeholder = '— Choisir —', disabled = false, accentColor = 'border-teal-500/40' }: CustomSelectProps) {
  ...
  return (
    <>
      <button
        ...
        className={`w-full flex items-center justify-between gap-2 bg-white/[0.06] border rounded-lg px-3 py-1.5 text-xs text-left transition-all disabled:opacity-40 disabled:cursor-not-allowed ${open ? `${accentColor} text-white` : 'border-white/[0.1] hover:border-white/[0.2]'}`}
      >
      ...
      {open && createPortal(
        <div ref={dropdownRef} style={{ ...style, background: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '0.75rem', overflow: 'hidden', boxShadow: '0 16px 48px rgba(0,0,0,0.7)' }}>
          <button type="button" onClick={() => { onChange(''); setOpen(false) }} className={`w-full flex items-center justify-between px-3 py-2 text-xs text-left transition-colors hover:bg-white/[0.06] ${!value ? 'text-teal-400' : 'text-slate-500'}`}>
            {placeholder}
            ...
            {options.length === 0
              ...
              : options.map(opt => (
                  <button key={opt.value} type="button" onClick={() => { onChange(opt.value); setOpen(false) }}
                    className={`w-full flex items-center justify-between px-3 py-2 text-xs text-left transition-colors hover:bg-white/[0.07] ${opt.value === value ? 'text-white bg-white/[0.04]' : 'text-slate-300'}`}>
...
// ── SubmissionImage — multi-pages avec navigation ─────────────────────────────
function SubmissionImage({ sub }: { sub: Submission }) {
  const [open, setOpen] = useState(false)
  const [page, setPage] = useState(0)

  // Construire la liste des URLs (multi-pages ou page unique)
  const total = sub.allImagePaths?.length ?? (sub.localImagePath ? 1 : 0)
  const imageUrls = Array.from({ length: total }, (_, i) => {
    const base = `${AGENT_URL}/admin/submissions/${sub._id}/image`
    return total > 1
      ? `${base}?index=${i}&key=${AGENT_KEY}`
      : `${base}?key=${AGENT_KEY}`
  })
  ...
  return (
    <>
      ...
      >
        <img src={imageUrls[0]} alt="copie" className="w-full h-full object-cover"
        ...
        {/* Badge multi-pages */}
        {total > 1 && (
          <div className="absolute bottom-1 right-1 flex items-center gap-0.5 bg-black/75 rounded-md px-1 py-0.5">
            <Layers className="w-2.5 h-2.5 text-teal-400" />
            <span className="text-[9px] text-teal-400 font-bold">{total}</span>
          </div>
        )}
      </button>
                ...
                <div className="flex items-center gap-3">
                  {total > 1 && (
                    <span className="text-xs text-slate-400">
                      Page <span className="text-white font-bold">{page + 1}</span> / {total}
                    </span>
                  )}
                  {sub.studentName && <span className="text-xs text-teal-400 font-semibold">{sub.studentName}</span>}
                  {sub.extractedCode && (
                    <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-mono bg-white/[0.08] text-slate-300 border border-white/[0.1]">
                      <Hash className="w-2.5 h-2.5" />{sub.extractedCode}
                    </span>
                  )}
                </div>
                <button onClick={() => setOpen(false)}
                  className="p-1.5 rounded-lg bg-white/[0.06] hover:bg-white/[0.14] text-slate-400 hover:text-white transition-all border border-white/[0.08]">
                  <X className="w-4 h-4" />
                </button>
              </div>

              {/* Image + navigation gauche/droite */}
              <div className="relative w-full flex items-center justify-center">
                {total > 1 && page > 0 && (
                  <button onClick={() => setPage(p => p - 1)}
                    className="absolute left-0 z-10 p-2 rounded-xl bg-black/60 hover:bg-black/80 text-white transition-all -translate-x-4 border border-white/[0.1]">
                ...
                <motion.img
                  key={page}
                  initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.15 }}
                  src={imageUrls[page]}
                  alt={`page ${page + 1}`}
                  className="max-h-[75vh] max-w-full rounded-xl object-contain shadow-2xl border border-white/[0.08]"
                />

                {total > 1 && page < total - 1 && (
                  <button onClick={() => setPage(p => p + 1)}
                    className="absolute right-0 z-10 p-2 rounded-xl bg-black/60 hover:bg-black/80 text-white transition-all translate-x-4 border border-white/[0.1]">
                ...
                <div className="flex items-center gap-2">
                  {imageUrls.map((_, i) => (
                    <button key={i} onClick={() => setPage(i)}
                      className={`rounded-full transition-all ${i === page ? 'w-5 h-2 bg-teal-400' : 'w-2 h-2 bg-white/20 hover:bg-white/40'}`}
                    />
                  ))}
                </div>
              )}
...
// ── SubmissionRow ─────────────────────────────────────────────────────────────
interface SubmissionRowProps { sub: Submission; exams: Exam[]; batches: Batch[]; index: number }
...
function SubmissionRow({ sub, exams, batches, index }: SubmissionRowProps) {
  ...
  function handleAssign() {
    assign.mutate(
      { submissionId: sub._id, examId: selectedExamId, examTitle: exams.find(e => e._id === selectedExamId)?.title ?? '', batchId: selectedBatchId || undefined },
      { onSuccess: () => setShowDropdowns(false) },
    )
  }

  function handleDelete() {
    deleteSub.mutate(sub._id, { onSuccess: () => setConfirmDelete(false) })
  }
    ...
    >
      <div className="flex gap-3 items-start">
        ...
        <div className="flex-1 min-w-0">
          {/* Badges + timestamp + bouton delete */}
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <div className="flex items-center gap-2 flex-shrink-0">
              <span className="text-[10px] text-slate-600">{timeAgo(sub.createdAt)}</span>
              {/* Bouton delete */}
              {!confirmDelete ? (
                <button
                  onClick={() => setConfirmDelete(true)}
                  className="p-1.5 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-500/60 hover:text-red-400 transition-all border border-red-500/10 hover:border-red-500/30"
                  title="Supprimer"
                >
                ...
                <div className="flex items-center gap-1">
                  ...
                  <button onClick={handleDelete} disabled={deleteSub.isPending}
                    className="px-2 py-1 rounded-lg bg-red-500/20 border border-red-500/30 text-[10px] text-red-300 font-bold hover:bg-red-500/30 transition-colors disabled:opacity-40">
                    {deleteSub.isPending ? <RotateCw className="w-3 h-3 animate-spin" /> : 'Oui'}
                  </button>
                  <button onClick={() => setConfirmDelete(false)}
                    className="px-2 py-1 rounded-lg bg-white/[0.04] border border-white/[0.08] text-[10px] text-slate-400 hover:text-white transition-colors">
                    Non
                  </button>
                </div>
              )}
              ...
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
                <div className="flex gap-2 mt-2 flex-wrap">
                  <div className="flex-1 min-w-[160px]">
                    ...
                    <CustomSelect value={selectedExamId} onChange={v => { setSelectedExamId(v); setSelectedBatchId('') }}
                      options={examOptions} placeholder="— Choisir un examen —" accentColor="border-teal-500/50" />
                  </div>
                  <div className="flex-1 min-w-[140px]">
                    <label className="block text-[9px] font-bold text-slate-500 mb-1 uppercase tracking-wider">Batch</label>
                    <CustomSelect value={selectedBatchId} onChange={setSelectedBatchId}
                      options={batchOptions} placeholder="— Nouveau batch —" disabled={!selectedExamId} accentColor="border-violet-500/50" />
                  </div>
                  <div className="flex items-end gap-1.5">
                    ...
                    <button onClick={handleAssign} disabled={!selectedExamId || assign.isPending}
    ...
    <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.05 }}
      ...
      {batch.status === 'open' && (
        <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}
          onClick={() => dispatch.mutate(batch._id)} disabled={dispatch.isPending}
      ...
      <motion.div key="session-backdrop"
        ...
        <motion.div key="session-card"
          ...
          <div className="flex items-center justify-between mb-1">
            ...
            <button onClick={onClose} className="w-7 h-7 rounded-lg bg-white/[0.04] border border-white/[0.08] flex items-center justify-center text-slate-400 hover:text-white transition-colors">
            ...
            <div className="flex items-center justify-between p-3 rounded-xl bg-emerald-500/10 border border-emerald-500/20 mb-4">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-slate-500">{session.receivedCount} reçues · {session.indexedCount} indexées · {session.failedCount} échecs</p>
                </div>
              </div>
          ...
          <div className="space-y-3">
            ...
            <CustomSelect value={selectedExamId} onChange={setSelectedExamId} options={examOptions}
              placeholder="— Sélectionner un examen actif —" accentColor="border-teal-500/50" />
            <div className="flex gap-2 pt-1">
              <button onClick={onClose} className="flex-1 py-2 rounded-xl bg-white/[0.04] border border-white/[0.08] text-xs text-slate-400 hover:text-white transition-colors">
                Annuler
              </button>
              <button onClick={() => selectedExam && activate.mutate(selectedExam)} disabled={!selectedExamId || activate.isPending}
            ...
            {activate.isError && (
              <p className="text-[10px] text-red-400 text-center">{(activate.error as any)?.response?.data?.message ?? 'Erreur activation'}</p>
            )}
          </div>
        </motion.div>
      </motion.div>
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => setShowSessionModal(true)}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold border transition-all hover:opacity-80 cursor-pointer ${
              session?.isActive ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-slate-500/10 border-slate-500/20 text-slate-400'
            }`}>
      ...
      <div className="flex gap-1 p-1 bg-white/[0.03] rounded-xl border border-white/[0.06] w-fit">
        {tabs.map(({ key, label, Icon, badge }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex items-center gap-1.5 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${tab === key ? 'bg-white/[0.08] text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>
            <Icon className="w-3.5 h-3.5" />
            {label}
            {badge > 0 && (
              <span className={`px-1.5 py-0.5 rounded-full text-[9px] font-bold ${tab === key ? 'bg-teal-500/20 text-teal-300' : 'bg-white/[0.06] text-slate-500'}`}>{badge}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {tab === 'inbox' && (
          <motion.div key="inbox" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
              ...
              {filters.map(f => (
                <button key={f.key} onClick={() => setFilter(f.key)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border ${
                    filter === f.key ? 'bg-white/[0.1] border-white/[0.15] text-white' : 'bg-white/[0.03] border-white/[0.06] text-slate-500 hover:text-slate-300'
                  }`}>
                  {f.label}
            ...
            {subsLoading ? (
              <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="glass-card p-4 animate-pulse h-24" />)}</div>
            ) : subs.length === 0 ? (
              <div className="glass-card p-10 text-center">
                <Inbox className="w-10 h-10 text-slate-700 mx-auto mb-3" />
                <p className="text-sm text-slate-500">{session?.isActive ? 'Aucune soumission pour ce filtre' : '⏸ Activez un examen pour recevoir des photos'}</p>
              </div>
            ) : (
              <div className="space-y-3">
                {subs.map((sub, i) => <SubmissionRow key={sub._id} sub={sub} exams={exams} batches={batches} index={i} />)}
              </div>
            )}
          </motion.div>
        )}
        ...
        {tab === 'batches' && (
          <motion.div key="batches" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-4">
            ...
            {batchLoading ? (
              <div className="space-y-3">{Array.from({ length: 3 }).map((_, i) => <div key={i} className="glass-card p-4 animate-pulse h-20" />)}</div>
            ) : batches.length === 0 ? (
              <div className="glass-card p-10 text-center">
              ...
              <div className="space-y-3">{batches.map((b, i) => <BatchCard key={b._id} batch={b} exams={exams} index={i} />)}</div>
            )}
          </motion.div>
        )}
      </AnimatePresence>
