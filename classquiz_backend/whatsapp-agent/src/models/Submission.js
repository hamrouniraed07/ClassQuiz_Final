 ...
 *
 * Une soumission = une photo envoyée par un parent via WhatsApp.
 *
 * Cycle de vie :
 *   received → code_extracted → student_found → queued → dispatched
 *                                                              ↓
 *                                                   OCR + grading dans ClassQuiz
 *
 * États d'échec :
 *   received → failed (si code absent, étudiant introuvable, téléchargement impossible)
 */
...
const submissionSchema = new mongoose.Schema({
  ...
  // 'no_code'         → aucun code dans la légende
  // 'invalid_code'    → format invalide
  // 'student_unknown' → code inconnu dans ClassQuiz
  // 'download_failed' → impossible de télécharger l'image
  // 'dispatch_failed' → erreur lors de l'envoi à ClassQuiz
